...
```
BLPOP BRPOP CLIENT CONFIG DBSIZE DISCARD EVAL  EVALSHA EXEC  MIGRATE MONITOR MOVE MULTI PSUBSCRIBE PUBLISH PUNSUBSCRIBE SCAN SCRIPT  SELECT  SLAVEOF SLOWLOG SMOVE SUBSCRIBE SYNC UNSUBSCRIBE UNWATCH WATCH BRPOPLPUSH
```
...
北京二可用区E提供具有高性能的分布式版Redis，经测试QPS性能达到同配置主备Redis的3倍，新用户创建的分布式版Redis默认是具有高性能的版本，目前公测中，老用户不受影响。具有高性能的分布式版Redis目前不支持部分协议（相比Redis3.2），具体如下：
...
分布式版Redis支持AOF文件重写功能，触发重写时，对较大的Redis可能造成短暂影响；为降低影响，我们降低了重写的频率，并将重写时间控制在凌晨3点至6点之间。
...
分布式redis扩容后，后台可能会对其进行增加分片的操作，从而增加处理能力，提高性能；
增加分片，是通过在线迁移实现，迁移时，会出现请求延迟增大现象，因此一般安排在凌晨；迁移操作，根据实际情况也可能会有几天的延迟，如果需要立刻迁移，可以通知下技术支持；

\#\# 主备版Redis短连接并发性能怎么样呢？ 使用工具redis\_benchmark进行压测，主备版Redis对于短连接的并发性能为1W
...
8G \< 容量 \< = 16G，redis自动重写阈值设为60G，在凌晨3点到6点之间，如果aof文件大于20G，强制重写；
