...
目前分布式Redis代理不支持的Redis协议具体如下（相比Redis 4.0，部分命令在代理最新版本支持，需要咨询技术支持后升级代理）：

```
CLUSTER COMMAND MIGRATE MOVE DUMP RANDOMKEY SELECT BGREWRITEAOF BGSAVE CLIENT CONFIG FLUSHALL FLUSHDB LASTSAVE MONITOR MEMORY ROLE PSYNC PFMERGE PFDEBUG PFSELFTEST SAVE SWAPDB SHUTDOWN SLAVEOF SLOWLOG SYNC TIME TOUCH
```
